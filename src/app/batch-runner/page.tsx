...
export default function BatchRunner() {
  ...
  const startBatch = async () => {
    ...
    while (true) {
      try {
        ...
        const res = await fetch(`/api/batch?mode=fins&from=${from}&size=3`);
  ...
  return (
    <div style={{minHeight:"100vh", background:C.bg, color:"#e2e8f0", padding:20, maxWidth:600, margin:"0 auto", fontFamily:"sans-serif"}}>
      <h1 style={{color:"#93c5fd", fontSize:18}}>🔄 バッチ自動実行（財務データのみ）</h1>
      <p style={{color:"#64748b", fontSize:13}}>ボタンを押すと全銘柄の財務データを自動取得してDBに保存します。途中でページを閉じないでください。</p>
