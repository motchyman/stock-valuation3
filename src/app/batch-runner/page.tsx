...
export default function BatchRunner() {
  ...
  const startBatch = async () => {
    ...
    while (true) {
      try {
        addLog(`処理中: ${from}〜${from + 49}件目...`, "info");
        const res = await fetch(`/api/batch?from=${from}&size=50`);
        ...
        setProgress(Math.round((from + 50) / total * 100));
        setStatus(`${from + 50} / ${total} 件 (${Math.round((from+50)/total*100)}%)`);
